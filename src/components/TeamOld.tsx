      ...
      <div className="flex flex-col md:flex-row w-full px-12 py-12 md:space-x-12 font-secondary">
        <div className="flex flex-col space-y-4 w-full md:w-2/6">
          ...
          <p className="text-white/70 text-lg">
            We are a team of {teamMembers.length} people who are passionate
          ...
          {teamMembers.map((member) => (
            <>
              <div className="flex group flex-row space-x-72 items-center justify-start w-full">
                <img
                  src={member.avatar}
                  className="w-64 group-hover:opacity-100 opacity-100 lg:opacity-80"
                />
                <div className="flex flex-col space-y-2">
                  <h1 className="text-7xl group-hover:text-white transition-all ease-linear cursor-default font-bold text-white lg:text-white/40">
                    {member.name}
                  </h1>
                  <div className="flex flex-row space-x-4">
                    {member.category?.map((category) => (
                      <p className="text-lg font-medium text-white/50 uppercase tracking-widest mt-4">
                        {category}
                      </p>
                    ))}
                  </div>
              ...
              {teamMembers.length - 1 !== member.id && (
